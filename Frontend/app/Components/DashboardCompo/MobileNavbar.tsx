...
function MobileNavbar({ open, setOpen }: Props) {
    ...
    return (
        <>
            ...
            <nav className={`md:hidden bg-white text-blue-950 w-80 h-screen fixed pt-80 left-0 bottom-0 mr-8 z-50 transform transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : '-translate-x-full'}`}>
                <button className='fixed top-8 right-3 text-[30px]' onClick={() => setOpen(false)}><IoIosClose /></button>
